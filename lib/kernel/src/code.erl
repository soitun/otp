...
%% set_primary_archive((FileName, ArchiveBin, FileInfo, MFA) -> ok | {error, Reason}
...
set_primary_archive(ArchiveFile0, ArchiveBin, #file_info{} = FileInfo, MFA)
  when is_list(ArchiveFile0), is_binary(ArchiveBin) ->
    ArchiveFile = filename:absname(ArchiveFile0),
    case call({set_primary_archive, ArchiveFile, ArchiveBin, FileInfo, MFA}) of
	{ok, []} ->
	    ok;
	{ok, _Mode, Ebins} ->
